...
class Game(pygame.sprite.Sprite):
    ...
    def __init__(self, surface):
        ...
        self.gameoperate = pygame.image.load(resource_path("pic//operate.png"))#储存操作提示图片
        self.StopImage = pygame.image.load(resource_path("pic//stop.png")) #储存暂停时显示图片
        self.score = 0    #分数
        ...
        self.menu = menu(self.surface, self)
        self.high_score = 0
        try:
    ...
    def whatmusic(self):    #场景切换时改变音乐
        ...
        if self.GamenotBegin and self.changeMusic:
            ...
            pygame.mixer.music.load(resource_path('music//start.ogg'))
            pygame.mixer.music.set_volume(0.2)
            pygame.mixer.music.play(-1)
        if self.GameOver and self.changeMusic:
            self.changeMusic = False
            pygame.mixer.music.load(resource_path('music//lost.ogg'))
            pygame.mixer.music.set_volume(0.2)
            pygame.mixer.music.play()
        if self.GameHelp and self.changeMusic:
            self.changeMusic = False
            pygame.mixer.music.load(resource_path('music//help.ogg'))
            pygame.mixer.music.set_volume(0.2)
            pygame.mixer.music.play(-1) 
        if not self.GamenotBegin and not self.GameOver and self.changeMusic:
            self.changeMusic = False
            pygame.mixer.music.load(resource_path('music//gaming.ogg'))
            pygame.mixer.music.set_volume(0.2)
            pygame.mixer.music.play(-1)
